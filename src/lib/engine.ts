...
export function analyze(
  ...
  if (hasHematoma && input.hematomaVolume !== undefined) {
    ...
    if (vol < 10 && !epiduralBigThick) {
      ...
      btfRules.push({ id: 'BTF-H1', name: 'BTF < 10ml', protocol: 'BTF', riskLevel: 'medium',
        description: `Gematoma ${vol}ml — konservativ, dinamik kuzatish zarur`, weight: 0.35 });
      ...
      if (gcsTotal >= 9) {
        btfScore = 85; surgicalUrgency = worstSurgical(surgicalUrgency, 'urgent');
        btfRules.push({ id: 'BTF-H3', name: 'BTF Jarrohlik', protocol: 'BTF', riskLevel: 'high',
          description: `Gematoma ${vol}ml + GCS ${gcsTotal} ≥ 9 — neyrojarroh maslahat zarur, jarrohlik ko'rsatmasi (BTF 2016)`, weight: 0.85 });
      } else {
  ...
  } else if (hasHematoma && input.hematomaVolume === undefined) {
    // Gematoma bor, lekin hajm kiritilmagan — ehtiyotkorlik sifatida monitor
    btfScore = Math.max(btfScore, 50);
    hematomaSurgery = hematomaSurgery ?? 'repeat_ct';
    surgicalUrgency = worstSurgical(surgicalUrgency, 'monitor');
    btfRules.push({ id: 'BTF-HVOL', name: 'Gematoma (hajm noma\'lum)', protocol: 'BTF', riskLevel: 'high',
      description: 'Gematoma aniqlangan, lekin hajm kiritilmagan — neyrojarroh maslahat zarur, hajm o\'lchansin (BTF 2016)', weight: 0.60 });
  }
  ...
  if (hasContusion) {
    ...
    hematomaSurgery = hematomaSurgery ?? 'repeat_ct';
    surgicalUrgency = worstSurgical(surgicalUrgency, 'monitor');
    // Gematoma ham bo'lsa BTF-MC-HC ustunroq — BTF-CONT chiqarilmaydi (chalkashlik oldini olish)
    if (!hasHematoma) {
      btfRules.push({
  ...
  if (hasFracture) {
    ...
    xaiEntries.push({
      ...
      impact: "Neyrojarroh maslahat zarur; asos suyagi sinishi belgilari bo'lsa — tezkor baholash zarur",
  ...
  if (hasHematoma && hasContusion) {
    ...
    xaiEntries.push({
      ...
      impact: "Neyrojarroh maslahat zarur; dinamik KT monitoring zarur",
      source: "BTF 2016, ACS TQIP 2023"
    });
  }
  ...
  if (hasHematoma && hasContusion && hasFracture) {
    ...
    xaiEntries.push({
      ...
      impact: "Neyrojarrohlik baholash zarur; hayot uchun xavfli holat — tezkor jarrohlik masalasi hal qilinsin",
  ...
  if (input.sex === 'female' && input.pregnancy) {
    niceScore += 20;
    niceRules.push({ id: 'NICE-P1', name: 'NICE Homiladorlik', protocol: 'NICE', riskLevel: 'high', description: "Homilador bemor: KT bajarilsa qorin himoyasi zarur; MRI afzal (NICE CG176)", weight: 0.20 });
  }
  ...
  if (input.alcoholIntoxication) {
    ...
    alcoholRules.push({ id: 'ALC-1', name: 'Alkogol intoksikatsiyasi', protocol: 'CCHR', riskLevel: 'medium',
      description: "Alkogol: GCS ishonchsiz bo'lishi mumkin → CT zarur; hushyor bo'lganda GCS qayta baholansin (Sperry 2006)", weight: 0.70 });
  }
  ...
  // overrideReasons doim qo'shiladi (urgency qatlamidan qat'iy nazar)
  if (input.anticoagulant) {
    if (hierarchyOverride === null) {
      hierarchyOverride = 'ct_mandatory';
    }
    // Sabab har doim ko'rsatilsin — antikoagulyant muhim klinik ma'lumot
    if (!overrideReasons.some(r => r.includes('Antikoagulyant + TBI'))) {
      overrideReasons.push("Antikoagulyant + TBI → CT zarur (Cohen 2006)");
    }
  }
  ...
  if (input.anticoagulant && gcsTotal < 15 && hierarchyOverride !== 'emergency' && hierarchyOverride !== 'urgent') {
    hierarchyOverride = 'urgent';
    surgicalUrgency = worstSurgical(surgicalUrgency, 'urgent');
    overrideReasons.push(`Antikoagulyant + GCS ${gcsTotal} < 15 — shoshilinch CT zarur (Cohen 2006, EFNS)`);
  }
  ...
  if (gcsTotal <= 8 && (hierarchyOverride === null || hierarchyOverride === 'ct_mandatory')) {
    hierarchyOverride = 'urgent';
    surgicalUrgency = worstSurgical(surgicalUrgency, 'urgent');
    overrideReasons.push(`GCS ${gcsTotal} <= 8 — og'ir TBI, darhol CT va neyrojarroh maslahat zarur`);
  }
  ...
  if (comorbidities.includes('coagulopathy') && hierarchyOverride === null) {
    hierarchyOverride = 'ct_mandatory';
    overrideReasons.push("Koagulopatiya + TBI → CT zarur (ACS TQIP 2023)");
  }
  ...
  if ((input.meningealSigns.kernig || input.meningealSigns.brudzinski || input.meningealSigns.neckStiffness)
    ...
    hierarchyOverride = 'urgent';
    surgicalUrgency = worstSurgical(surgicalUrgency, 'urgent');
    overrideReasons.push("Meningeal belgilar + gematoma → neyrojarroh maslahat zarur (NICE 2023, BTF 2016)");
  }

  // K6: Alkogol + GCS <= 12 (Sperry 2006, NICE 2023)
  if (input.alcoholIntoxication && gcsTotal <= 12 && hierarchyOverride === null) {
    hierarchyOverride = 'ct_mandatory';
    overrideReasons.push(`Alkogol + GCS ${gcsTotal} <= 12 → CT zarur, GCS ishonchsiz bo'lishi mumkin (Sperry 2006)`);
  }
  ...
  if (comorbidities.includes('coagulopathy') && hasHematoma && hierarchyOverride !== 'emergency') {
    hierarchyOverride = 'urgent';
    surgicalUrgency = worstSurgical(surgicalUrgency, 'urgent');
    overrideReasons.push("Koagulopatiya + gematoma → reversal terapiya va neyrojarroh maslahat zarur (ACS TQIP 2023)");
  }

  // ── DARAJA 3: O'RTA XAVF ─────────────────────────────────────────────

  if (input.anticoagulant && ctNormal && hierarchyOverride === null) {
    overrideReasons.push("Antikoagulyant + CT normal → 24 soat yotqizish zarur (Cohen 2006)");
  }
  ...
  if ((input.meningealSigns.kernig || input.meningealSigns.brudzinski || input.meningealSigns.neckStiffness) && ctNormal) {
    overrideReasons.push("Meningeal belgilar + CT normal → LP zarur (SAQ ehtimolini istisno qilish)");
  }
  if (input.sex === 'female' && input.pregnancy) {
    overrideReasons.push("Homilador bemor: CT bajarilsa qorin himoyasi + akusher koordinatsiya");
  }
  if (input.age >= 65 && gcsTotal >= 13) {
    overrideReasons.push("65+ yosh + GCS 13–15 → CCHR yuqori xavf, CT zarur");
  }
  ...
  if (input.complaints.seizure && gcsTotal <= 12) {
    overrideReasons.push("Tutqanoq + GCS <= 12 → CT shoshilinch zarur (BTF 2016)");
  }
  ...
  // Confidence max: 0.35 + 0.25 + 0.15 + 0.17 + 0.05 = 0.97 → 0.95 cap ishlatiladi
  // score*0.17 (oldin 0.08 edi — 0.95 ga hech qachon yetilmас edi)
  const confidence = Math.round(
    Math.min(0.95, Math.max(0.35,
  ...
  if (hierarchyOverride === 'emergency' || surgicalUrgency === 'emergency') {
    ...
    if (surgicalUrgency === 'urgent') {
      ...
      // CT bajarilgan + urgent override (GCS<13, vital) + surgical=not_required
      // → SURGICAL_EVALUATION: bemor kuzatish va neyrojarroh maslahat zarur
      decision = 'SURGICAL_EVALUATION';
  ...
  if (surgicalUrgency === 'emergency') {
    ...
    treatmentTactics = [
      "Neyrojarroh bilan maslahat zarur",
      "6–12 soatdan keyin KT takrorlansin",
      "Neyrojarrohlik bo'limiga yotqizish zarur",
      "Har soatda nevrologik monitoring o'tkazilsin",
    ];
    ...
    treatmentTactics = [
      "KT bajarilishi zarur",
      "Kuzatish uchun yotqizish zarur",
    ...
    treatmentTactics = [
      ...
      "Zarur bo'lsa neyrojarroh bilan maslahat o'tkazilsin",
    ];
  }
  ...
  if (rtsScore !== undefined && rtsScore < 5) {
    treatmentTactics.push(
      isHighUrgency
        ...
        : `RTS ${rtsScore} (norma 7.84) — travma markazi aktivatsiyasi baholansin (Boyd 1987)`
    );
  }
  ...
  if (input.alcoholIntoxication)
    treatmentTactics.push(
      isHighUrgency
        ...
        : "Alkogol: GCS ishonchsiz bo'lishi mumkin — 4–6 soatdan keyin klinik qayta baholash zarur (NICE 2023)"
    );
  if (input.sex === 'female' && input.pregnancy)
    treatmentTactics.push("Homilador: akusher-ginekolog bilan koordinatsiya; KT bajarilsa qorin sohasini qo'rg'oshin ekran bilan himoya qilish zarur");
  ...
  if ((input.meningealSigns.kernig || input.meningealSigns.brudzinski || input.meningealSigns.neckStiffness) && ctNormal)
    treatmentTactics.push("Meningeal belgilar + CT normal: LP (lyumbar punksiya) zarur — SAQ ehtimolini istisno qilish");
  ...
  if (comorbidities.includes('stroke'))
    treatmentTactics.push("Insult tarixi: antitrombotik dorilar mavjud bo'lsa reversal zarur; bazal GCS ni aniqlash zarur");
  if (comorbidities.includes('dementia'))
    treatmentTactics.push("Demensiya: bazal GCS ma'lum bo'lsa shu bilan taqqoslash zarur; oila / parvarish xodimidan ma'lumot olish zarur");
  if (comorbidities.includes('heart_failure'))
    treatmentTactics.push("Yurak yetishmovchiligi: MAP monitoring zarur; suyuqlik yuklashda ehtiyotkorlik zarur");
  ...
  if (comorbidities.includes('cancer'))
    treatmentTactics.push("Onkologik kasallik: metastaz ehtimoli — MRI zarur");
  if (comorbidities.includes('renal'))
    treatmentTactics.push("Buyrak yetishmovchiligi: kontrastli KT da ehtiyotkorlik zarur; dori dozalarini moslashtirish zarur");
  ...
  if (cchrScore >= 35) {
    ...
    xaiEntries.push({
      ...
      impact: "CCHR: o'rta xavf = KT zarur, ayniqsa yosh ≥65 yoki antikoagulyant bo'lsa",
  ...
  if (hasHematoma && input.hematomaVolume !== undefined && input.hematomaVolume > 0) {
    ...
    xaiEntries.push({
      ...
      impact: `BTF 2016: subdural >10mm yoki siljish >5mm; epidural qalinlik ≥15mm yoki siljish >5mm — jarrohlik mezonlari. Hozir: ${vol >= 30 || shift > 5 ? "jarrohlik ko'rsatmasi ehtimoli yuqori" : vol >= 10 ? "kuzatish + takror KT zarur" : "konservativ boshqaruv zarur"}`,
  ...
  if (input.anticoagulant) {
    xaiEntries.push({
      ...
      impact: "Cohen 2006 (n=20): dastlab normal KT bo'lgan bemorlarning 90% qayta qabul, jiddiy qon ketish rivojlangan. EFNS: antikoagulyant = mustaqil KT ko'rsatmasi, GCS dan qat'iy nazar. KT normal bo'lsa ham 24 soat yotqizish zarur",
  ...
  if (input.meningealSigns.kernig || input.meningealSigns.brudzinski || input.meningealSigns.neckStiffness) {
    ...
    xaiEntries.push({
      ...
      impact: "NICE CG176: meningeal belgilar = KT va LP zarur. LP faqat KT dan keyin — intrakranial bosimni avval istisno qilish zarur",
  ...
  if (input.alcoholIntoxication) {
    xaiEntries.push({
      ...
      impact: "Sperry 2006 (n=55,732): alkogol GCS ni o'rtacha 0.8 ball pasaytiradi. NICE 2023: alkogol metabolizmlanguncha (4–6 soat) qayta baholash zarur",
  ...
  if (input.age >= 65) {
    xaiEntries.push({
      ...
      impact: "CCHR: yosh ≥65 = yuqori xavf mezoni. NICE CG176: 65+ + antikoagulyant = KT zarur. BTF: yosh ≥60 jarrohlik xavfini oshiradi",
